            ...
            users.map((user)=>{
                return (                
                    <tr>
                        ...
                        <td>                            
                            {user.qualities.map((quality=>(<span className={renderBadge(quality.color)}> {quality.name} </span>)))}
                        </td>
                        <td>{user.profession.name}</td>
                        <td>{user.completedMeetings}</td>
                        <td>{user.rate}/5</td>
                        <td><span className='btn btn-danger' onClick={()=>handleDelete(user._id)}>Удалить</span></td>
                    </tr>   
                )
